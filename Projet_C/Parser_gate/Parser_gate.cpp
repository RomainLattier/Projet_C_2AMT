      ...
      vector<string> *v_out,const string * s_path) {
        ...
        while(getline(infile, ligne)){
          ...
          // ou X est le nombre de caractere da,s la recherche find()
            ...
            Gate_mux * ptr_obj = new Gate_mux(nom,nb_entry,nb_sel);
            ...
            vector<Gate *> * ptr_v_in = new vector<Gate *>();
            cout << "Création de l'entrée "<< sel_name << endl;
            ptr_v_in->push_back(ptr_obj);
            m_input->insert(pair<string,vector<Gate*>* >(sel_name,ptr_v_in) );
